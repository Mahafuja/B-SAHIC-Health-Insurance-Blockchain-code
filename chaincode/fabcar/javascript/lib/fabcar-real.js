...
class FabCar extends Contract {
    ...
    async initLedger(ctx) {
        ...
        for (let i = 0; i < cars.length; i++) {
            ...
            let temp = i + 1210001;
            await ctx.stub.putState(temp.toString(), Buffer.from(JSON.stringify(cars[i])));
    ...
    async cashless(ctx, policy_no, treatmentapplyingfor) { // add treatment record to a user
        ...
        if (result.lifetimesupport >= ((treatementprice[treatmentapplyingfor]) * availpercentage[treatmentapplyingfor] / 100)) {
            ...
            await ctx.stub.putState(policy_no, Buffer.from(JSON.stringify(car)));
    ...
    async createCar(ctx, policy_no, ssn, name, age, gender, contact, medical_history, occupation, nominees_details, issue_date, maturity_date, plan_year, address) {
        ...
        await ctx.stub.putState(nextfreepolicyno.toString(), Buffer.from(JSON.stringify(car)));
        console.info('============= END : Create Policy ===========');
        return (nextfreepolicyno);
    }
